# !/usr/bin/env python written by @jts from
# https://github.com/jts/ncov2019-artic-nf/blob
# /be26baedcc6876a798a599071bb25e0973261861/bin/process_gvcf.py
...
# from https://www.geeksforgeeks.org/python-make-a-list-of-intervals
# -with-sequential-numbers/ via artic-mask
...
# write the depth mask used with bcftools to turn consensus positions
# into Ns
def write_depth_mask(out_filename, contig_depths, min_coverage):
    ...
    for contig_name, depths in contig_depths.items():
        # from artic-mask, create list of positions that fail the
        # depth check
        mask_vector = []
...
# calculate the variant allele fraction for each alt allele using
# freebayes' read/alt observation tags
...
# make a simple VCF record with the minimal information needed to
# make the consensus sequence
def make_simple_record(vcf_header, parent_record, position, ref, alt,
                       vaf):
...
# process indel variants found by freebayes into a variant that
# should be applied to the consensus sequence
def handle_indel(vcf_header, record):
    output = list()
    vafs = calculate_vafs(record)

    # special case, if we have evidence for multiple possible indels
    # (eg CTTT -> C, CTTT -> CT) we decide whether to apply an indel
    # based on the summed VAF across all alt alleles, then apply the
    # most frequent ALT. This is because there is evidence for /an/
    # indel but it is ambiguous which one. We can't represent
    # ambiguous indels in a consensus fasta so this is the best we
    # can do.
    if sum(vafs) < 0.5:
        return output
    ...
    r = make_simple_record(vcf_header, record, record.pos,
                           record.ref, record.alts[max_idx], [max_vaf])
    output.append(r)
    return output
...
def handle_sub(vcf_header, record):
    ...
    for i in range(0, sub_length):
        ...
        if max_b is None:
            continue
        r = make_simple_record(vcf_header, record, record.pos + i,
                               record.ref[i], max_b, base_frequency[
                                   i][max_b])
        output.append(r)
    return output
...
def main():
    ...
    parser.add_argument('-c', '--consensus-sites-output', required=True,
                        help=f"The output file name for variants that "
                             f"will be applied to generate the "
    ...
    parser.add_argument('-l', '--lower-ambiguity-frequency', type=float,
                        default=0.25,
                        help=f"Variants with frequency less than -l "
                             f"will be discarded")

    parser.add_argument('-u', '--upper-ambiguity-frequency', type=float,
                        default=0.75,
                        help=f"Substitution variants with frequency "
                             f"less than -u will be encoded with "
                             f"IUPAC ambiguity codes")
    ...
    for record in vcf:
        ...
        has_indel = False
        for i in range(0, len(record.alts)):
            has_indel = has_indel or len(record.ref) != len(
                record.alts[i])
