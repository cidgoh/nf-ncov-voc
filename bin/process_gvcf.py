...
@author: @jts from
https://github.com/jts/ncov2019-artic-nf/blob/be26baedcc6876a798a599071bb25e0973261861/bin/process_gvcf.py

This script is written by Jared in his ncov-2019-artic-nf repo to
process gvf foile produced by Freebayes variant caller. This script
is used in nf-ncov-voc workflow.
...
# fromhttps://www.geeksforgeeks.org/python-make-a-list-of-intervals-with-sequential-numbers/
# via artic-mask
...
def handle_indel(vcf_header, record):
    ...
    # (eg CTTT -> C, CTTT -> CT)
    # we decide whether to apply an indel based on the summed VAF
    # across all alt alleles, then
    # apply the most frequent ALT. This is because there is evidence
    # for /an/ indel but it is
    # ambiguous which one. We can't represent ambiguous indels in a
    # consensus fasta so this
...
def main():
    ...
    parser.add_argument('-c', '--consensus-sites-output',
                        required=True, help=f"The output file name "
    ...
    parser.add_argument('-l', '--lower-ambiguity-frequency',
                        type=float, default=0.15, help=f"Variants "
                                                       f"with "
                                                       f"frequency "
                                                       f"less than -l "
                                                       f"will be "
    ...
    parser.add_argument('-u', '--upper-ambiguity-frequency',
                        type=float, default=0.75, help=f"Substitution "
                                                       f"variants "
                                                       f"with "
                                                       f"frequency "
                                                       f"less than -u "
