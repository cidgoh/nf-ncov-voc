...
[![DOI](https://zenodo.org/badge/366519671.svg)](https://doi.org/10.5281/zenodo.16988884)
...
To adapt this workflow for a new virus, users must provide a defined set of input files. These are processed through three modular components:  
**🧪 Genomics Workflow**, **🧬 Functional Annotation**, and **📊 Visualization**.
...
> `genome_config.JSON` is the most important file for adapting the workflow to a new virus.  
...
![Workflow Diagram](figs/VIRUS-MVP-ADAPT.png)

## 🧬 Format Standards, Conversion Logic, and Validation

The workflow supports multiple tools for variant calling and sequence alignment through a modular and configurable structure. While this flexibility enables broad adoption and customization, it also introduces complexity in managing heterogeneous output formats and ensuring interoperability.

To address these challenges, we have implemented a systematic approach to format standardization, including:

### ✅ Supported Formats

- **Sequences:**
  - `FASTA` - standard format for sequence files
- **Metadata:**

  - `TSV` – standardized sample-level annotations

- **Variant Calling Input/Output:**

  - `VCF` – standardized variant format
  - `GVF` – downstream annotation and visualization

- **Sequence Alignment Output:**
  - `BAM` / `CRAM` – alignment data
...
- Raw `iVar TSV` outputs are parsed and converted into `VCF` using internal conversion script [TSV to VCF conversion script](./bin/ivar_variants_to_vcf.py).
- `VCF` files are normalized and transformed into `GVF` format while preserving key annotations such as `AF`, `DP`, `ALT`, and `REF` using the script [VCF to GVF conversion script](./bin/vcf2gvf.py).
- During each transformation, header fields, sample identifiers, and variant-specific annotations are preserved to ensure compatibility.

---

### 🔍 Validation Steps

We include a **VCF validation module** in the workflow to ensure correctness and completeness of format conversions:

- Schema-aware checks (e.g., required `INFO` fields, formatting compliance)
- Field-level validation to confirm that essential data (e.g., allele frequency, reference/alt alleles) is retained post-conversion
- Logging of errors and warnings for any fields dropped or altered during transformation

These validation steps help ensure interoperability between workflow components and reduce the risk of downstream inconsistencies caused by tool-specific formats.

---

### 🧩 Schema and Standards References

- [VCF 4.3 Specification](https://samtools.github.io/hts-specs/VCFv4.3.pdf)
- [GVF Specification](https://www.sequenceontology.org/resources/gvf.html)

---

### 🔮 Future Directions: Schema-Driven Integration

To further improve interoperability, we are actively exploring the use of [LinkML](https://linkml.io/) to define schema-driven, virus-agnostic output formats. This will enable consistent annotation, validation, and exchange of variant and metadata information across tools, workflows, and databases.
