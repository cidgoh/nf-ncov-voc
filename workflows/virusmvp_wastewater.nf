    ...
    main:
        ...
        if (!params.skip_fastp) {
            if (params.adapter_fasta) { adapter = file(params.adapter_fasta) 
                WW_FASTP (
                    ch_short_reads, adapter, params.save_trimmed_fail, params.save_merged
                    )
                }
            else{
                WW_FASTP (
                    ch_short_reads, [], params.save_trimmed_fail, params.save_merged
                    )  
                }
        ...
        human_genome = [
                    [ id:params.host_genome_id, single_end:true ], // meta map
                    file(params.host_genome, checkIfExists: true)
                ]
        viral_genome = [
                    [ id:params.viral_genome_id, single_end:true ], // meta map
